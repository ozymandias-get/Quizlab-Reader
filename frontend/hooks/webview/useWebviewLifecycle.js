...
export function useWebviewLifecycle({
    ...
    const [webviewElement, setWebviewElement] = useState(null)
    ...
    const webviewMethods = useMemo(() => ({
        executeJavaScript: (script) => activeWebviewRef.current?.executeJavaScript(script),
        getActiveWebview: () => activeWebviewRef.current,
        getWebview: () => activeWebviewRef.current,
        insertText: (text) => activeWebviewRef.current?.insertText(text),
        ...
        sendInputEvent: (...args) => activeWebviewRef.current?.sendInputEvent(...args),
        paste: () => activeWebviewRef.current?.paste(),
        getWebContentsId: () => activeWebviewRef.current?.getWebContentsId(),
        pasteNative: async (id) => {
            if (id && window.electronAPI?.forcePaste) {
                return await window.electronAPI.forcePaste(id);
            }
            return false;
        },
        addEventListener: (event, handler, options) => activeWebviewRef.current?.addEventListener(event, handler, options),
        removeEventListener: (event, handler, options) => activeWebviewRef.current?.removeEventListener(event, handler, options),
    ...
    const handleFailLoad = useCallback((event) => {
        setIsLoading(false)
        if (event.errorCode === ERROR_CODE_ABORTED) return
        setError(event.errorDescription || t('page_load_failed'))
    }, [t])
    ...
    const handleCrashed = useCallback(() => {
        ...
        if (crashRetryCount.current < MAX_CRASH_RETRIES) {
            ...
            timeoutRef.current = setTimeout(() => {
                if (activeWebviewRef.current === crashedWebview) {
                    activeWebviewRef.current?.reload()
                }
            }, CRASH_RETRY_DELAY)
        } else {
    ...
    const onWebviewRef = useCallback((element) => {
