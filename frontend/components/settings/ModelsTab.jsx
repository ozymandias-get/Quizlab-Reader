...
function ModelsTab() {
    ...
    const modelsList = Object.keys(aiSites)
    ...
    const handleAddAi = async (e) => {
        ...
        try {
            ...
            if (result && result.success) {
                ...
                let currentEnabled = [...enabledModels]
                if (!currentEnabled.includes(result.id)) {
    ...
    const handleDeleteAi = async (e, id, name) => {
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="relative">
                                            ...
                                            `}>
                                                {getAiIcon(site?.icon || key) ? React.cloneElement(getAiIcon(site?.icon || key), { className: "w-5 h-5" }) : <GridIcon className="w-5 h-5" />}
                                            </div>
                                            {/* Status Dot */}
                                            {isEnabled && (
                                                <motion.div
                                        ...
                                        <div className="space-y-0.5">
                                            <div className="flex items-center gap-2">
                                                <Label className={`text-sm font-bold transition-colors duration-300 ${isEnabled ? 'text-white' : 'text-white/50'}`}>
                                                    {site.displayName || (key.charAt(0).toUpperCase() + key.slice(1))}
                                                </Label>
                                                {isCustom && (
